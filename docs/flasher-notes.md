...
6. 写入 36 帧“未连接”闪烁离线动图、DHCP 失败状态图、获取中背景、IP 背景，共 `3900` 页。
...
离线动图来自 `flasher/src-tauri/assets/offline_animation.rgb565be`，这是一个 36 帧合并 RGB565BE 资源，每帧 `25600` 字节。固件固定按约 `100ms`/帧播放，flasher 只负责把 36 帧写到 `0, 100, 200, ... 3500` 页。当前动画为 9 帧显示“未连接”、9 帧空背景，重复两轮。
...
上述日志来自加入 `DHCP失败` 状态图之前的实机验证；当前固定写入计划为 `images=39 pages=3900`。
