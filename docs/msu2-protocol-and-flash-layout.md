...
| 页范围 | 页数 | 内容 |
| --- | ---: | --- |
| `0..3599` | `3600` | 36 帧待机动图，每帧 100 页；当前写入“未连接”显示/消失硬切闪烁动画 |
| `3726..3825` | `100` | DHCP 失败状态图 |
| `3826..3925` | `100` | 获取中背景 |
| `3926..4025` | `100` | IP 背景 |
| `4026+` | 保留 | 官方字库/单色图/其他资源，不写入 |

官方指南说明 MINI 离线状态会循环播放 36 张彩色图，每 100ms 切一帧，合计 3.6 秒。当前 flasher 使用合并的 `offline_animation.rgb565be` 资源切出 36 帧；背景和边框保持不动，只让“未连接”文字按 0.9 秒显示、0.9 秒消失的节奏硬切闪烁。
