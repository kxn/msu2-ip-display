...
| `3726..3825` | `DHCP失败` 状态图 |
| `3826..3925` | 未获取 IP / 获取 IP 中状态图 |
| `3926..4025` | IP 显示空白背景 |
| `4026+` | 官方 `N24X33P` 数码管资源，必须保留 |

`3726..3825` 使用了当前固定写入布局里的空档。官方资料曾在 `3600..3825` 一带放过大号数码管、ASCII 字库或 Logo 示例资源；当前 IP 显示方案只依赖 `4026+` 的 24x33 数码管，因此这个取舍可接受，但需要在文档里持续标注。
...
1. **未获取 IP**：目标网络接口没有可用 IPv4，显示 page `3826`。
2. **DHCP 失败**：只拿到 `169.254.0.0/16` link-local IPv4，显示 page `3726`。
3. **获得 IP**：拿到正常 IPv4，显示 page `3926` 背景，并用官方 `N24X33P` 数码管显示 IP。

设备未插入时，程序不显示任何内容，只保持监听；屏幕自身会回到已刷入的未连接/待机动图。

内部状态可以比屏幕状态更细，例如“等待 DHCP 稳定”“多个地址但未确定 DHCP 来源”。这些内部状态在 v1 都继续显示 page `3826`，只有确认失败后才切到 page `3726`。
