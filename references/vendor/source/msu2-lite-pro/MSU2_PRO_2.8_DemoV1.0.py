...
def Write_NAND_Flash_block(block_add):#读取指定地址的数值
    hex_use=bytearray()#空数组
    hex_use.append(3)#对Flash操作
    hex_use.append(4)#高速写入
    hex_use.append(0)
    block_add=block_add%(256*256)
    hex_use.append(block_add//256)
    hex_use.append(block_add%256)
    hex_use.append(0)
...
def Get_Photo_Path2():#获取文件路径
    ...
    photo_path2=tk.filedialog.askopenfilename(title="选择文件",filetypes=[('wav音频文件','*.wav')])
...
def Writet_Photo_Path1():#写入文件
    ...
    if write_path1==0:#确保上次执行写入完毕
        ...
        if (im1.width*LCD_Y)>=(im1.height*LCD_X):#图片长宽比例超过4:3
            im2=im1.resize((int(LCD_Y*im1.width/im1.height),LCD_Y))
            Img_m=int(im2.width/2)
            box=((Img_m-LCD_X//2,0,Img_m+LCD_X//2,LCD_X))#定义需要裁剪的空间
            im2=im2.crop(box)
        else:
            im2=im1.resize((LCD_X,int(LCD_X*im1.height/im1.width)))
            Img_m=int(im2.height/2)
            box=((0,Img_m-LCD_Y//2,LCD_X,Img_m+LCD_Y//2))#定义需要裁剪的空间
            im2=im2.crop(box)
...
def Write_Flash_wav_music_fast(Page_add,Photo_name):#往Flash里面写入Bin格式的照片
    global Text1
    filepath=Photo_name+'.wav'#合成文件名称
    
    try:#尝试打开bin文件
        #binfile=open(filepath,'rb')#以只读方式打开
        music=wave.open(filepath,'rb')#以只读方式打开
    except:#出现异常
        #print('找不到“'+filepath+'”文件,请检查其位置是否位于当前目录下');
        #Text1.delete(1.0,END)#清除文本框
        Text1.insert(END,'文件路径或格式出错!\n')#在文本框开始位置插入“内容一”
        return 0
    f=music.getframerate()
    n=music.getnframes()
    
    
    if (f!=8000) :
        Text1.insert(END,'格式错误,采样率需设置为8000!\n')
        OUT=n*8000//f#计算采样后的数据
        
        original_indices = np.linspace(0, n-1, OUT)#计算比例
        data=music.readframes(n)#读出全部数据
        
        np.interp(original_indices, np.arange(len(data)), data)
        
        hex_use=bytearray()#空数组
        for i in range(0,n):#逐字解析编码
            data16=music.readframes(1)
            num=data16[1]
            if(num>127):
                num=num-128
            else:
                num=num+128
            hex_use.append(num)#添加数据
            if(i%10000==0):
                LCD_Set_XY(0,0)#保持连接，防止断线
...
def LCD_Photo(LCD_X,LCD_Y,LCD_X_Size,LCD_Y_Size,Page_Add):#
    global Device_State
    H_Page=Page_Add//65536
    Page_Add=Page_Add%65536
    ...
    hex_use=hex_use+int(Page_Add%256).to_bytes(1,byteorder="little")
    hex_use=hex_use+int(H_Page).to_bytes(1,byteorder="little")
    SER_Write(hex_use)#发出指令
    #等待收回信息
    while(1):
        time.sleep(0.001)
...
def LCD_PWM(PWM_duty):#控制背光亮度
    ...
    hex_use=int(2).to_bytes(1,byteorder="little")#对LCD多次写入
    hex_use=hex_use+int(3).to_bytes(1,byteorder="little")#设置指令
    hex_use=hex_use+int(18).to_bytes(1,byteorder="little")#载入地址
    hex_use=hex_use+int(PWM_duty//256).to_bytes(1,byteorder="little")
    hex_use=hex_use+int(PWM_duty%256).to_bytes(1,byteorder="little")
    hex_use=hex_use+int(0).to_bytes(1,byteorder="little")
    SER_Write(hex_use)#发出指令
...
def show_gif():#显示GIF动图
    ...
    if(State_change==1):
        ...
        time.sleep(0.01)
    if(State_change==0):
        #LCD_Photo(0,0,LCD_X,LCD_Y,75*8*gif_num+512*8)
        ...
        #LCD_Add_RAM(0,0,LCD_X,172,1024*256+gif_blk_num*430*16)
        ...
        LCD_Load_RAM_Mix_Show(YELLOW,75*8*gif_num+512*8)
...
def show_PC_state(FC,BC):#显示PC状态
    ...
    if(State_change==0):
        Add_X=(LCD_X-160)//2
        Add_Y=(LCD_Y-80)//2
        LCD_Add_RAM(0,(LCD_Y-172)//2,320,172,1024*256+page_now1*430*16)#放置动图
        ...
        LCD_Load_RAM_Mix_Show(FC,0x71800+256*4)
...
def show_Photo1():#显示照片
    ...
    if(State_change==1):
        ...
        LCD_RAM_Init(0x00)#清空缓冲区
        #LCD_State(LCD_Change_now)#配置显示方向
        #LCD_Photo(0,0,LCD_X,LCD_Y,512*8)#Page
        LCD_Photo(0,0,LCD_X,LCD_Y,0x71800)#Page
...
def show_PC_time():
    ...
    if(State_change==1):
        ...
        LCD_RAM_Init(0x00)#清空缓冲区
        #LCD_State(LCD_Change_now)#配置显示方向
        LCD_Photo(0,0,LCD_X,LCD_Y,photo_add*8)#Page
...
def UI_Page():#进行图像界面显示
    ...
    #创建主窗口
    root = tk.Tk() # 实例化主窗口
    root.title("MSU2_PRO屏幕助手V1.0")#设置标题
    ...
    btn4=tk.Button(root,text="选择WAV音频",height=1,width=12)
...
def Erase_Flash_Chip():#128KB
    hex_use=bytearray()#空数组
    hex_use.append(3)#对Flash操作
    hex_use.append(5)#Erase 
    hex_use.append(0)
    hex_use.append(0)
    hex_use.append(0)
    hex_use.append(0)
    SER_Write(hex_use)#发出指令
    #print('Erase'+str(block_add))
    #等待收回信息
    while(1):
        time.sleep(0.0001)
        recv = SER_Read()#.decode("UTF-8")#获取串口数据
...
def Erase_NAND_Flash_Block(Block_Num):#擦除128KB区块0~1023
    hex_use=bytearray()#空数组
    hex_use.append(3)#对Flash操作
    hex_use.append(6)#Erase 
    hex_use.append(Block_Num//256)
    hex_use.append(Block_Num%256)
    hex_use.append(0)
    hex_use.append(0)
    SER_Write(hex_use)#发出指令
...
def Get_MSN_Device():#尝试获取MSN设备
    ...
    if len(port_list) == 0:
        ...
        if(len(My_MSN_Device)>=1):
            ...
            print("全片擦除耗时："+str(time.time()-e_time1))
...
def MSN_Device_1_State_machine():#MSN设备1的循环状态机
    ...
    if(write_path1==1):
        ...
        #time.sleep(3);
        #Write_NAND_Flash_hex_fast(0,Img_data_use)
        #write_path1=0
        State_change=1
        ...
        LCD_State(LCD_Change_now)#配置显示方向
        #进行擦除
        for i in range(910,912):#清除所有视频
            Erase_NAND_Flash_Block(i//1)#128KB/Nlock
        ...
        Write_NAND_Flash_block(910*64)#要装入的Page
        im1=cv2.imread(photo_path1)
        if im1 is None:
            print(f"错误：无法读取图片 '{photo_path3}'")
        height = im1.shape[0]
        width = im1.shape[1]
        if(width*LCD_Y>=height*LCD_X):#适配不同分辨率的屏幕，让其充满小屏幕显示区域
            size_USE_Y1=height
            ...
            size_USE_Y1=size_USE_Y1+size_USE_Y0
            print(size_USE_X0,size_USE_Y0,size_USE_X1,size_USE_Y1)
        im = im1[size_USE_Y0:size_USE_Y1, size_USE_X0:size_USE_X1]
        im2=cv2.resize(im, (LCD_X,LCD_Y), interpolation=cv2.INTER_AREA)#高质量图像缩小
        # 提取通道并转换为uint16避免溢出
        r = im2[:, :, 2].astype(np.uint16)  # BGR中的R通道
        g = im2[:, :, 1].astype(np.uint16)  # G通道
        b = im2[:, :, 0].astype(np.uint16)  # B通道
        RGB= ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3)
        hex_16RGB=RGB.reshape(-1)#转成u16形式
        rgb_ptr = hex_16RGB.ctypes.data_as(POINTER(c_uint16))#指针转换
        buf_prt = cast(buffer_data, POINTER(c_ubyte))
    ...
    if(write_path2==1):
        ...
        for i in range(928,960):
            Erase_NAND_Flash_Block(i//1)#125KB/Nlock
        
        #Write_Flash_Photo_fast(0x74000,photo_path2)#按Page写入，音乐地址
        Write_Flash_wav_music_fast(0x74000,photo_path2)
        #Write_Flash_Photo_fast(0,photo_path2)#按Page写入，初始地址
        write_path2=0
        State_change=1
    ...
    if(write_path3==1):
        ...
        Write_NAND_Flash_block(908*64)#要装入的Page
        im1=cv2.imread(photo_path3)
        if im1 is None:
            print(f"错误：无法读取图片 '{photo_path3}'")
        height = im1.shape[0]
        width = im1.shape[1]
    ...
    if(write_path4==1):
        ...
        buffer_size = LCD_X*LCD_Y*10
        buffer_data = create_string_buffer(buffer_size)
        Write_NAND_Flash_block(512)#要装入的Page
        Path_use=photo_path4
        if Path_use[-4]=='.':
            write_path4=Path_use[-4:]
            Path_use=Path_use[:-5]
        ...
        if(width*LCD_Y>=height*LCD_X):#适配不同分辨率的屏幕，让其充满小屏幕显示区域
            ...
            size_USE_Y1=size_USE_Y1+size_USE_Y0
            print(size_USE_X0,size_USE_Y0,size_USE_X1,size_USE_Y1)
        for i in range(0,768):#依次转换64张图片
            #u_time2=time.time()
            im1=cv2.imread(Path_use+str(i)+write_path4)
            #u_time2=time.time()-u_time2
            #print('读取耗时'+str(int(u_time2*1000))+'ms\n')
            #u_time2=time.time()
            im = im1[size_USE_Y0:size_USE_Y1, size_USE_X0:size_USE_X1]
            im2=cv2.resize(im, (LCD_X,LCD_Y), interpolation=cv2.INTER_AREA)#高质量图像缩小
            # 提取通道并转换为uint16避免溢出
            ...
            uart_data=bytes(buffer_data[:num])
            #u_time2=time.time()-u_time2
            #print('转换耗时'+str(int(u_time2*1000))+'ms\n')
            #u_time2=time.time()
            ser.write(uart_data)
            #u_time2=time.time()-u_time2
            #print('烧录耗时'+str(int(u_time2*1000))+'ms\n')
        u_time=time.time()-u_time
        Text1.insert(END,'烧写完成,耗时'+str(int(u_time*1000))+'ms\n')#在文本框开始位置插入“内容一”
        State_change=1
    if(State_machine==0):
